...
class SokobanLevel:
    def __init__(self, width: int, height: int, num_boxes: int, boundaries: List[Tuple[int, int]] = None, start_pos: Tuple[int, int] = None):
        self.width = width
        self.height = height
        self.targets = []
        self.boxes = []
        ...
        for x, y in self.boundaries:
            if 0 <= x < height and 0 <= y < width:
                self.nodes[x][y].wall = False  # Changed from True to False
                self.nodes[x][y].tile_id = 0   # Floor tile ID
    ...
    def is_player_trapped(self):
        ...
        for dx, dy in directions:
            nx, ny = self.playerX + dx, self.playerY + dy
            if (0 <= nx < self.height and 0 <= ny < self.width and
                not self.nodes[nx][ny].wall):
                return False
        return True
    ...
    def randomSpot(self):
        available_spots = [spot for spot in self.allowedSpots if not spot.permanent and not spot.occupied]
        if available_spots:
    ...
    def rip(self, amount):
        if self.is_player_trapped():
            ...
            for dx, dy in directions:
                nx, ny = self.playerX + dx, self.playerY + dy
                if (0 <= nx < self.height and 0 <= ny < self.width and
                    self.nodes[nx][ny].wall and not self.nodes[nx][ny].permanent):
                    self.nodes[nx][ny].wall = False
                    amount -= 1
                    break
    ...
    # Helper function to check if a node is blocking (box or permanent wall)
        def is_blocking(x, y):
            if not (0 <= x < self.height and 0 <= y < self.width):
                return False  # Out of bounds is not blocking
            return self.nodes[x][y].has_box or self.nodes[x][y].permanent

        # Check right side (X+1 direction)
        if X+1 < self.height and is_blocking(X+1, Y):
            # Check top-right and right
            if (is_blocking(X+1, Y+1) and is_blocking(X, Y+1)):
                return True
            # Check bottom-right and right
            if (is_blocking(X+1, Y-1) and is_blocking(X, Y-1)):
                return True

        # Check left side (X-1 direction)
        if X-1 >= 0 and is_blocking(X-1, Y):
            # Check top-left and left
            if (is_blocking(X-1, Y+1) and is_blocking(X, Y+1)):
                return True
            # Check bottom-left and left
            if (is_blocking(X-1, Y-1) and is_blocking(X, Y-1)):
                return True
